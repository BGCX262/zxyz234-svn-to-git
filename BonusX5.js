...
//
//Open Multiple Links in RD passed me somecode from spockholm so the function that opens the links inside mafia wars
//is there source and credit goes to them.

//http://dl.dropbox.com/u/111164001/BONUS%20%20X%205/FSBonusX5-3.js
...
(function () {
	...
	function process_links() {
		...
		function process(link_no) {
			...
			if (url.length > link_no) {
				...
				//flp $('#link_process_no').text(url.length+' links processed '+run_times+' times each')
	...
	function unshort(url, handler) {
		...
		if (/apps.facebook.com\/inthemafia/.test(url)) {
			...
			$.getJSON('http://api.longurl.org/v2/expand?url=' + escape(url) + '&user-agent=Application-Name%2F3.7&format=json&callback=?',
				function (data) {
	...
	function request_fburl(url, handler, errorhandler) {
		...
		url = url + strparams;
		var timestamp = parseInt(new Date().getTime().toString().substring(0, 5));
		if (url.indexOf('cb=') == -1) {
